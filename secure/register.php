...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["firstName"])) || !(preg_match("/^[a-zA-Z ]*$/", $_POST["firstName"]))) {
        $firstName_err = "First name must conatin only letters and cannot be empty.";
    } else {
    ...
    // Validate Last Name
    if (empty(trim($_POST["lastName"])) || !(preg_match("/^[a-zA-Z ]*$/", $_POST["lastName"]))) {
...
// Validate password
    ...
    mysqli_query($link, "INSERT INTO `ip-registration` (`address` ,`timestamp`)VALUES ('$ip',CURRENT_TIMESTAMP)");
    $result = mysqli_query($link, "SELECT COUNT(*) FROM `ip-registration` WHERE `address` LIKE '$ip' AND `timestamp` > (now() - interval 1 week)");
    $count = mysqli_fetch_array($result, MYSQLI_NUM);

    if ($count[0] > 1) {
        $email_err = "You can register one user per week";
    }
    ...
    if (empty($email_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssss", $email, $firstName, $lastName, $param_password);
            // Set parameters
            //****      $param_username = $username;
            $param_password = password_hash($password, PASSWORD_DEFAULT); // password hashing with random salt
                    ...
                    <div class="grid_5">
                        ...
                        <form id='contact-form' class='contact-form'
                            ...
                            <div class="form-group <?php echo (!empty($email_err)) ? 'has-error' : ''; ?>">
                                <label>Email</label>
                                <input required id="emailregister" type="text" name="email" class="form-control"
                            ...
                            <div class="form-group <?php echo (!empty($firstName_err)) ? 'has-error' : ''; ?>">
                                ...
                                <input required="true" id="firstnameregister" type="text" name="firstName"
                                       class="form-control" value="<?php echo $firstName; ?>" pattern="^([a-zA-Z]+)$"
                                       title="spiceal chars are not allowed">
                                <span class="error-message-simple"><?php echo $firstName_err; ?></span>
                            </div>

                            <div class="form-group <?php echo (!empty($lastName_err)) ? 'has-error' : ''; ?>">
                                <label>Last name</label>
                                <input required="true" id="lastnameregister" type="text" name="lastName"
                                       class="form-control" value="<?php echo $lastName; ?>" pattern="^([a-zA-Z]+)$"
                                       title="spiceal chars are not allowed">
                            ...
                            <div class="form-group <?php echo (!empty($password_err)) ? 'has-error' : ''; ?>">
                                ...
                                <input required="true" id="passwordregister" type="password" name="password"
                            ...
                            <div class="form-group <?php echo (!empty($confirm_password_err)) ? 'has-error' : ''; ?>">
                                ...
                                <input required="true" id="confirmregister" type="password" name="confirm_password"
